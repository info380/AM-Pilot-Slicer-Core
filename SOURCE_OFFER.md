...
3. SHA-256 checksums for both archives;
4. an OCI SPDX SBOM and build provenance extracted from the immutable image
   attestations;
5. signature-verification, vulnerability-scan, dependency-notice, binary
   checksum, and evidence-checksum files;
6. a release-evidence JSON document tying those artifacts to the digest.
